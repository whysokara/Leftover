...
1a. `OnboardingView` (`showOnboarding`) — the 3-step first-run flow; while it's up, `homeView` never appears, so the photo-permission request waits for the step-3 primer
...
- In-app favoriting was removed (July 2026): no double-tap gesture, no dock star — already-favorited photos just show a passive star badge. If any future feature mutates asset properties via `performChanges`, remember `PHAsset` objects are immutable snapshots: re-fetch by `localIdentifier` afterward and swap the refreshed asset into `photoAssets`/`currentAsset`.
