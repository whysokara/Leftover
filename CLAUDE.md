...
- In-app favoriting (re-added): double-tap the swipe card (or tap the heart in the action dock) to toggle the photo's Photos-library favorite via `toggleFavorite()`. The dock heart reflects the current photo's state (hollow `heart` → filled `heart.fill` on `chipPink`), a matching heart badge sits on a favorited card, and turning a favorite on blooms a heart over the photo (`favoriteBurst`, off under Reduce Motion). `PHAsset` is an immutable snapshot, so `toggleFavorite` re-fetches by `localIdentifier` after the `performChanges` write and swaps the refreshed asset back into `photoAssets`/`currentAsset` — do the same for any future property mutation. **Favorite writes need a real device** — the simulator sandboxes photo-library writes (same reason deletes fail there).
